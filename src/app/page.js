...
export default function Home() {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            const response = await fetch(`https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=1&${api}`, options)
    ...
    return(
        <div>
            <NavBar />
            <div>
                {movies.length > 0 ? <Carrusel movies={movies} /> : <p>Cargando...</p>}
            </div>
            <TopRated topMovies={results} />
        </div>
    )
}
